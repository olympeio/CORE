...
describe('HTTPDelete function brick', () => {
    it('should delete correctly', async () => {
        ...
        await brick.update(context, ['https://httpbin.org/delete', '{"test": "payload"}', '{"Content-Type": "application/json"}'], outputs);
    ...
    it('should generate a 405 error when deleting on a put-only URL', async () => {
        const brick = new HTTPDelete();
        const context = new BrickContext().createChild();
        const outputs = [
            jasmine.createSpy(),
    ...
    it('should generate a 404 error when deleting on a wrong url',  async () => {
        const brick = new HTTPDelete();
        const context = new BrickContext().createChild();
        const outputs = [
            jasmine.createSpy(),
